...
func (c networkPageComponent) Render(_ context.Context, w io.Writer) error {
	...
	for _, r := range restaurants {
		...
		b.WriteString("<p class=\"font-semibold text-slate-800 flex items-center gap-2\">")
		b.WriteString(html.EscapeString(r.Name))
		if isCurrent {
			b.WriteString("<span class=\"text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full font-medium\">Ваш заклад</span>")
			b.WriteString("<button type=\"button\" hx-get=\"/admin/network/restaurants/")
			b.WriteString(strconv.Itoa(r.ID))
			b.WriteString("/edit\" hx-target=\"#admin-modal-content\" hx-swap=\"innerHTML\" class=\"p-1 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors\" onclick=\"event.stopPropagation()\" title=\"Редагувати заклад\">")
		...
		for _, role := range roles {
			...
			if len(people) == 0 {
				...
				for _, p := range people {
					...
					if isCurrent {
						deleteURL := "/admin/network/staff/" + p.Role + "/" + strconv.Itoa(p.ID)
						b.WriteString("<div class=\"flex items-center gap-1\">")
						...
						b.WriteString("<svg class=\"w-4 h-4\" fill=\"none\" stroke=\"currentColor\" viewBox=\"0 0 24 24\"><path stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2\" d=\"M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z\"/></svg>")
						b.WriteString("</button>")
						b.WriteString("<button type=\"button\" data-delete-url=\"")
						b.WriteString(deleteURL)
						b.WriteString("\" data-delete-label=\"")
						b.WriteString(html.EscapeString(p.Name))
						b.WriteString("\" onclick=\"adminConfirmDelete(this.dataset.deleteUrl,this.dataset.deleteLabel)\" class=\"p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors\">")
						b.WriteString("<svg class=\"w-4 h-4\" fill=\"none\" stroke=\"currentColor\" viewBox=\"0 0 24 24\"><path stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2\" d=\"M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16\"/></svg>")
						b.WriteString("</button>")
						b.WriteString("</div>")
					}
					b.WriteString("</div>")
		...
		if len(tables) == 0 {
			...
			for _, t := range tables {
				...
				if isCurrent {
					tableDeleteURL := "/admin/network/tables/" + strconv.Itoa(t.ID)
					tableLabel := "Столик №" + strconv.Itoa(t.Number)
					b.WriteString("<div class=\"flex items-center gap-1\">")
					...
					b.WriteString("</button>")
					b.WriteString("<button type=\"button\" data-delete-url=\"")
					b.WriteString(tableDeleteURL)
					b.WriteString("\" data-delete-label=\"")
					b.WriteString(tableLabel)
					b.WriteString("\" onclick=\"adminConfirmDelete(this.dataset.deleteUrl,this.dataset.deleteLabel)\" class=\"p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors\">")
					b.WriteString("<svg class=\"w-4 h-4\" fill=\"none\" stroke=\"currentColor\" viewBox=\"0 0 24 24\"><path stroke-linecap=\"round\" stroke-linejoin=\"round\" stroke-width=\"2\" d=\"M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16\"/></svg>")
					b.WriteString("</button>")
					b.WriteString("</div>")
