...
type NetworkServicer interface {
	...
	CreateStaff(role, name, phone, pin string, restaurantID int) error
...
func (s *NetworkService) CreateStaff(role, name, phone, pin string, restaurantID int) error {
	...
	case "chef":
		specID, err := s.repo.GetDefaultChefSpecializationID()
		if err != nil {
			return err
		}
		return s.repo.CreateChef(name, phone, pinHash, restaurantID, specID)
	case "admin":
